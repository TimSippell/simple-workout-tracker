...
struct ExercisesView: View {
    ...
    var body: some View {
        List {
            ...
            .onDelete { indexSet in
                for index in indexSet {
                    OwtBridge.shared.deleteExercise(id: exercises[index].id)
                }
                reload()
            }
        }
        ...
        .alert("Get Started", isPresented: $showSeedAlert) {
            Button("Add defaults") {
                OwtBridge.shared.seedDefaultExercises()
                reload()
                if !OwtBridge.shared.isSetupComplete() {
                    showSetupAlert = true
                }
            }
        ...
        .sheet(isPresented: $showAddSheet) {
            ExerciseFormSheet(title: "Add Exercise") { name, category, muscle, trackingType in
                _ = OwtBridge.shared.addExercise(name: name, category: category, muscleGroup: muscle, notes: trackingType)
                reload()
            }
        }
        .sheet(item: $editingExercise) { exercise in
            ExerciseFormSheet(
                ...
                OwtBridge.shared.updateExercise(id: exercise.id, name: name, category: category, muscleGroup: muscle, notes: trackingType)
                reload()
            }
        }
    }
