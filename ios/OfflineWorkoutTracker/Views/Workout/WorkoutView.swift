...
struct WorkoutView: View {
    ...
    var body: some View {
        ...
        .alert("Cancel Workout", isPresented: $showCancelAlert) {
            Button("Delete", role: .destructive) {
                if let id = activeWorkoutId { OwtBridge.shared.deleteWorkout(id: id) }
                resetWorkout()
            }
        ...
        .alert("Incomplete Workout", isPresented: $showFinishAlert) {
            Button("Finish") {
                if let id = activeWorkoutId { OwtBridge.shared.finishWorkout(id: id) }
                resetWorkout()
            }
        ...
        .sheet(isPresented: $showTemplatePicker) {
            TemplatePickerSheet { templateId in
                activeWorkoutId = OwtBridge.shared.startWorkoutFromTemplate(templateId: templateId)
                exercises = OwtBridge.shared.listExercises()
                refreshSets()
                showActive = true
            }
    ...
    private var startView: some View {
        VStack(spacing: 12) {
            if activeWorkoutId != nil {
                ...
                Button {
                    activeWorkoutId = OwtBridge.shared.startWorkout()
                    exercises = OwtBridge.shared.listExercises()
                    showActive = true
                } label: {
    ...
    private func restoreActiveWorkout() {
        if let w = OwtBridge.shared.getActiveWorkout() {
    ...
    private func refreshSets() {
        if let id = activeWorkoutId {
            sets = OwtBridge.shared.getSetsForWorkout(workoutId: id)
        }
    }
        ...
        NavigationStack {
            Form {
                ...
                Section("Details") {
                    ...
                    TextField("Weight (\(OwtBridge.shared.getWeightUnit()))", text: $weight)
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Add") {
                        ...
                        if let matched = matchedExercise {
                            ...
                            id = OwtBridge.shared.addExercise(name: exerciseName, category: "", muscleGroup: "", notes: "weight")
                        }
